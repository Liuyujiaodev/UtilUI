...
- (void)alertErrorMsg:(id)dic {
    if ([dic isKindOfClass:[NSDictionary class]]) {
        [Util showAlertView:self title:@"" message:[dic stringWithKey:@"msg"] okAction:@"" cancelAction:@"我知道了" okHandler:nil
                cancelHandler:nil];
    } else if ([dic isKindOfClass:[NSString class]]) {
        [Util showAlertView:self title:@"" message:dic okAction:@"" cancelAction:@"我知道了" okHandler:nil
                cancelHandler:nil];
    }
...
- (void)requestError {
    if (!self.isAlertShwo) {
        [Util showAlertView:self title:@"网络连接失败" message:@"请检查您的网络权限以及网络开关，下拉界面重新加载" okAction:@"好" cancelAction:@"设置" okHandler:^(UIAlertAction * _Nullable action) {
            self.isAlertShwo = NO;
